...
from src.agent_memory import load_memory, memory_summary, record_outcome
from src.cache import get_cached_search_results, record_search
...
from src.llm import OllamaError, chat, resolve_model
...
def tool_list_uncrawled_candidates(limit: int = 10) -> list[dict]:
    from src.run import _load_candidates
...
def tool_save_candidates(rows: list[dict], phase: str = "A") -> int:
    from src.run import _append_candidates
