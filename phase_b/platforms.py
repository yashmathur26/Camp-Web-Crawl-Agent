...
from phase_c.junk_audit import is_unusable_name
...
async def _fetch(
    ...
    *,
    ...
    from phase_b.crawl import fetch_page_text_and_links
...
async def _harvest_program_slug_catalog(url: str, links: list[dict]) -> list[dict]:
    ...
    def absorb(link_list: list[dict]) -> None:
        for link in link_list:
            ...
            from phase_a.camp_discovery import score_camp_candidate
...
async def adapter_webtrac(url: str, links: list[dict], page_text: str) -> list[dict]:
    ...
    """
    from phase_b.camp_validator import extract_registration_sessions
    ...
    for cat in catalogs[:5]:
        ...
        if not parsed_rows:
            from phase_b.camp_validator import extract_webtrac_search_results
...
async def adapter_sawyer(url, links, page_text):
    ...
    from phase_b.crawl import fetch_rendered
...
async def adapter_llm(url: str, links: list[dict], page_text: str, town_hint: str = "") -> list[dict]:
    ...
    from phase_b.camp_validator import extract_camp_sessions, should_llm_extract
    from phase_b.enrollment_signals import attach_inline_verification, verify_registrable
...
async def _trail_before_llm(url: str, town_hint: str) -> tuple[str, list[dict], str]:
    ...
    from config.settings import SETTINGS
    from phase_b.camp_validator import make_link_follow_gate
    ...
    best = url
    best_score = 0
    from phase_b.registration import registration_url_priority
...
def _apply_focus_llm_tiebreaker(
    ...
    *,
    ...
    from config.settings import SETTINGS
    from phase_b.camp_validator import verify_youth_summer
    from shared.llm import OllamaError, is_available
    from phase_a.relevance import is_ambiguous, is_hard_drop
    ...
    def _camp_context(s: dict) -> bool:
        ...
        from phase_c.junk_audit import has_evidence
        from phase_b.registration import is_registration_platform_url
...
def _links_from_html(base_url: str, html: str) -> list[dict]:
    ...
    from shared.urls import to_absolute
...
async def _enumerate_provider_v2(url: str, *, town_hint: str = "") -> dict:
    ...
    from config.settings import SETTINGS
    from shared import session_log
    from phase_b.navigator import navigate_provider
    from phase_a.relevance import is_ambiguous, is_hard_drop
    ...
    if SETTINGS.get("filter_to_focus"):
        from phase_a.relevance import filter_sessions
...
async def enumerate_provider(url: str, *, town_hint: str = "") -> dict:
    ...
    from phase_a.camp_discovery import collect_camp_candidate_links, discover_verified_sessions
    from shared import session_log as _session_log
