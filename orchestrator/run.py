...
from shared.cache import get_cached_search_results, load_seen_urls, record_search
from phase_a.candidate_outputs import write_candidates_txt
from phase_a.classify import classify
from phase_a.camp_hosts import is_camp_host_seed_url, is_rec_center_host
from phase_b.camp_validator import filter_rows_with_llm, make_link_follow_gate
from phase_b.crawl import fetch_page_text, harvest_guide_outbound, walk_site
from phase_b.crawl_scheduler import crawl_profile, seed_tier, should_skip_seed, sort_seeds
from phase_b import harvest_activity, harvest_log
from phase_a.discover import ConfigError, search
from phase_a.filter_links import is_external_camp_lead, is_likely_camp_link
from phase_a.link_quality import is_quality_camp_link
from shared.geo_filter import is_out_of_state_url
from phase_b.registration import is_registration_platform_url, registration_url_priority
from shared import data_layout
from shared.csv_mirror import mirror_csv_to_txt
...
def _run_one_search(
    ...
    if SETTINGS.get("phase_a_llm_judge"):
        from phase_c.agent_tools import _llm_judge_results
...
def _run_phase_a_budgeted(
    ...
    from phase_a.search_budget import YieldStopper, prioritized_queries, town_budget
...
def _run_quality_gate(town_name: str, results: list[dict]) -> None:
    from phase_b.session_quality import enrollability_score, write_quality_csvs
...
def _run_registration_trail(town_name: str, results: list[dict]) -> None:
    from phase_b.registration_trail import merge_trail_sessions, run_registration_trail
...
def _run_parent_verify(
    ...
    *,
    ...
    from phase_b.parent_verify import run_parent_verify
...
def _run_gap_fill(
    ...
    *,
    ...
    from phase_c.gap_finder import run_gap_fill
...
def _warmup_ollama_models() -> None:
    if not SETTINGS.get("ollama_preload_models"):
        return
    from shared.llm import chat, is_available, resolve_model
...
def _run_session_enumeration(
    ...
    *,
    ...
    from phase_c.deliverables import write_deliverables
    ...
    from phase_c.pilot_analysis import update_pilot_analysis
...
def main() -> None:
    ...
    if args.reset_links and not dry_run:
        from shared.store import DEFAULT_CAMP_LINKS_PATH
    ...
    try:
        if args.agent:
            from phase_c.agent import run_agent
        ...
        if run_phase == "Q" and town_filter and not dry_run:
            from phase_b.sessions import enumerate_town, write_session_outputs
        ...
        if run_phase == "trail" and town_filter and not dry_run:
            from phase_b.sessions import enumerate_town
            results = asyncio.run(enumerate_town(town_filter[0]))
        ...
        if run_phase == "D" and not dry_run:
            from phase_d.part_d import run_part_d
        ...
        if run_phase in ("gap", "C") and not dry_run and (args.all_towns or args.towns):
            from phase_c.part_c import run_part_c
