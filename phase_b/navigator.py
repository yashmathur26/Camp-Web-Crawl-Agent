...
from phase_b.registration import (
    crawl_link_score,
    is_camp_catalog_url,
...
def find_register_link(page_url: str, links: list[dict]) -> str:
    ...
    """
    from phase_c.junk_audit import is_offhost_register
...
async def extract_one_camp(
    ...
    *,
    ...
    if not reg and ok:
        from phase_b.camp_validator import extract_camp_sessions
...
async def _verify_session(
    ...
    from phase_b.enrollment_signals import can_verify_inline
    ...
    *,
    ...
    async def fetch_page(url: str, *, kind: str = "") -> tuple[str, list[dict]]:
        ...
        if town_hint and text.strip():
            from phase_b.capture import capture_page
    ...
    while queue and fetches < fetch_cap:
        ...
        if role == PageRole.CATALOG:
            ...
            if not camp_links and fetches < fetch_cap:
                from phase_b.platforms import adapter_llm
