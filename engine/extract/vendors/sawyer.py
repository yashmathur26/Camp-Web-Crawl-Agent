...
class SawyerExtractor(Extractor):
    ...
    async def extract(self, provider: Provider, fetch: FetchClient) -> ExtractResult:
        ...
        if not rows and provider.org_id:
            ...
            # Card headings carry the camp names (link text is "Join Waitlist"
            # chrome — LexFarm probe) — same heading→set pairing as marketing pages.
            rows = pair_headings_to_sets(shtml)
        if not rows:
